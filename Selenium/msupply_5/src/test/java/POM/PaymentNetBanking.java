...
public class PaymentNetBanking 
{
	
    Logger log = LogReports.writeLog(PaymentNetBanking.class);
	...
	@FindBy(xpath="//a[@id='tab_paymenthdfcnb_standard']/label[text()='NetBanking ']")
	...
	public void NetBanking() throws IOException, InterruptedException 
	{ 
		...
		log.info("Click on NetBanking Option");
		...
		//Step 2 : Calculate Payment Charges 2.06% for Net Banking and Validate
		if(ActualSubTotalValue<1000)
		{	
		   //Calculate the Service charge for SubTotal below 1000 rupees, 2.06% is added for Credit card
			//Service charge is 10% of Subtotal+14.5% of(10% of subtotal)+2.06% of subtotal
			
		   float ConvienceFee1=10*(ActualSubTotalValue)/100;
		   float ConvienceFee2=(float) (14.5*(ConvienceFee1)/100);
		   float CreditcardPaymentCharges=(float) (2.06*(ActualSubTotalValue)/100);
		...
		{
			//Calculate the Service charge for SubTotal above 1000 rupees, 2.06% is added for NetBanking
			//Service charge is 100+14.5%(100)+2.06% of subtotal
			
		   float ConvienceFee1=100;
		   float ConvienceFee2=(float) (14.5*(ConvienceFee1)/100);
		   float CreditcardPaymentCharges=(float) (2.06*(ActualSubTotalValue+ActualShippingChargesvalue+ActualVATValue)/100);
		   float ExpectedServiceCharge=ConvienceFee1+ConvienceFee2+CreditcardPaymentCharges;
		   log.info("ExpectedServiceCharge without removing decimal"+ExpectedServiceCharge);
		   try
		   {

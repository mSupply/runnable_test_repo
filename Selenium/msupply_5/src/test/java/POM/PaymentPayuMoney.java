...
public class PaymentPayuMoney 
{
	     Logger log = LogReports.writeLog(PaymentPayuMoney.class);
		...
		@FindBy(xpath="//a[@id='tab_paymentpayucheckout_shared']/label[text()='PayUmoney ']")
		WebElement PayuMoneyOption;
		...
		public void PayuMoney() throws IOException, InterruptedException 
		{ 
			...
			log.info("Click on PayuMoney Option");
			PayuMoneyOption.click();
			...
			//Step 2 : Calculate Payment Charges 2.29% for PayuMoney and Validate
			if(ActualSubTotalValue<1000)
			{	
			   //Calculate the Service charge for SubTotal below 1000 rupees, 2.29% is added for PayuMoney
				//Service charge is 10% of Subtotal+14.5% of(10% of subtotal)+2.29% of subtotal
				
			   float ConvienceFee1=10*(ActualSubTotalValue)/100;
			   float ConvienceFee2=(float) (14.5*(ConvienceFee1)/100);
			   float CreditcardPaymentCharges=(float) (2.29*(ActualSubTotalValue)/100);
			...
			{
				//Calculate the Service charge for SubTotal above 1000 rupees, 2.29% is added for PayuMoney
				//Service charge is 100+14.5%(100)+2.29% of subtotal

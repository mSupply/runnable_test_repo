...
class Mage_Catalog_Model_Product_Api_V2 extends Mage_Catalog_Model_Product_Api
{
    ...
    public function info($productId, $store = null, $attributes = null, $identifierType = null)
    {
		...
		$result['image_url'] = $product->getImageUrl();
		// $result['spec']['specs'] = $product->getData('specs');
		// $result['spec']['grade'] = $product->getData('grade');
		// $result['spec']['color'] = $product->getData('color');
		$result['spec'] = array('specs' => $product->getData('specs'),'grade' => $product->getData('grade'), 'color' => $product->getAttributeText('color'));
		$block  = Mage::getModel('cms/block')
				->load('our_quality_promise');
		$result['our_quality_promise'] = $block->getContent();
		$block  = Mage::getModel('cms/block')
				->load('our_return_policy');
		$result['our_return_policy'] = $block->getContent();
        return $result;
    }

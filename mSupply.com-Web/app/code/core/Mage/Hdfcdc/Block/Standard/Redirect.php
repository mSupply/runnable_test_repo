...
class Mage_Hdfcdc_Block_Standard_Redirect extends Mage_Core_Block_Abstract
{

    protected function _toHtml()
    {
        $standard = Mage::getModel('hdfcdc/standard');
        $form = new Varien_Data_Form();
        $form->setAction($standard->getHdfcUrl())
            ->setId('hdfcdc_standard_checkout')
            ->setName('hdfcdc_standard_checkout')
            ->setMethod('POST')
            ->setUseContainer(true);
		
		$secret_key = Mage::getSingleton('hdfcdc/config')->getSecretKey();
		if(Mage::getSingleton('hdfcdc/config')->getTransactionMode() == 1) {
			$mode = "TEST";
		...
		$hashType = Mage::getSingleton('hdfcdc/config')->getHashType();
		$pageId = Mage::getSingleton('hdfcdc/config')->getPageId();

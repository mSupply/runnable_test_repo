...
class Mage_Hdfcdc_StandardController extends Mage_Core_Controller_Front_Action
{
    /**
     * Order instance
     */
    ...
    {
        return Mage::getSingleton('hdfcdc/config')->getDebug();
    }
    ...
    public function redirectAction()
    {
        ...
        $order->addStatusToHistory(
            $order->getStatus(),
            Mage::helper('hdfcdc')->__('Customer was redirected to Hdfc')
        );
        $order->save();

        $this->getResponse()
            ->setBody($this->getLayout()
                ->createBlock('hdfcdc/standard_redirect')
                ->setOrder($order)
                ->toHtml());
    ...
    {
    	$secret_key = Mage::getSingleton('hdfcdc/config')->getSecretKey(); // Your Secret Key
    	$hashType = Mage::getSingleton('hdfcdc/config')->getHashType(); // Your Secret Key
    	$params = $secret_key;
    	$response = $_POST;    	
    	$secureHash = $response['SecureHash'];		
    	$ResPaymentId = $response['PaymentID'];	//Payment Id
		$ResTrackID =$response['MerchantRefNo'];        //Merchant Track ID
        $ResTranId = $response['TransactionID'];           //Transaction ID
		Mage::log("track id".$ResTrackID."Payment Id".$ResPaymentId."Transaction Id".$ResTranId);
    	...
    	if($response['ResponseCode'] == 0 && $hashValid) {
            ...
            $order->setHdfcTransid($ResTranId);
    		if (!$order->getId()) {
    			$this->norouteAction();
    			return;
    		}
    		...
    		$order->addStatusToHistory(
	    		$order->getStatus(),
	    		Mage::helper('hdfcdc')->__('Customer successfully returned from Hdfc')
    		);
    	...
    	{
    		$this->_redirect('hdfcdc/standard/failure');
    	}
    }
    ...
    public function notifyAction ()
    {
        ...
        if ($this->getDebug()) {
            $debug = Mage::getModel('hdfcdc/api_debug');
    ...
    {
        $errorMsg = Mage::helper('hdfcdc')->__('There was an error occurred during paying process.');

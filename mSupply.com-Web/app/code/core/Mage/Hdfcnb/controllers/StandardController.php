...
class Mage_Hdfcnb_StandardController extends Mage_Core_Controller_Front_Action
{
    /**
     * Order instance
     */
    ...
    {
        return Mage::getSingleton('hdfcnb/config')->getDebug();
    }
    ...
    public function redirectAction()
    {
        ...
        $order->addStatusToHistory(
            $order->getStatus(),
            Mage::helper('hdfcnb')->__('Customer was redirected to Hdfc')
        );
        $order->save();

        $this->getResponse()
            ->setBody($this->getLayout()
                ->createBlock('hdfcnb/standard_redirect')
                ->setOrder($order)
                ->toHtml());
    ...
    {
    	$secret_key = Mage::getSingleton('hdfcnb/config')->getSecretKey(); // Your Secret Key
    	$hashType = Mage::getSingleton('hdfcnb/config')->getHashType(); // Your Secret Key
    	$params = $secret_key;
    	$response = $_POST;    	
    	...
    	if($response['ResponseCode'] == 0 && $hashValid) {
    		...
    		$order->addStatusToHistory(
	    		$order->getStatus(),
	    		Mage::helper('hdfcnb')->__('Customer successfully returned from Hdfc')
    		);
    	...
    	{
    		$this->_redirect('hdfcnb/standard/failure');
    	}
    }
    ...
    public function notifyAction ()
    {
        ...
        if ($this->getDebug()) {
            $debug = Mage::getModel('hdfcnb/api_debug');
    ...
    {
        $errorMsg = Mage::helper('hdfcnb')->__('There was an error occurred during paying process.');

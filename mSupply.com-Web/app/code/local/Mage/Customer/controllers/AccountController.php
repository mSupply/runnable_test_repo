...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    {
		$sessionmsg = Mage::getSingleton("core/session",  array("name"=>"frontend"));
            $sessionmsg->setData("loginmsg",'');
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    $sessionmsg->setData("loginmsg",$message);
   ...
   protected function _loginPostRedirect()
{
    ...
    public function createPostAction()
    {
        ...
        $dataname =  $this->getRequest()->getPost('firstname');
		$emaildata =$this->getRequest()->getPost('email');
		//if($emaildata == ''){
		//  $emaildata =$dataname.'@gmail.com';
		//}else{
		$emaildata =$this->getRequest()->getPost('email');
		//}
		//$emaildata =$this->getRequest()->getPost('email');
        $customer = Mage::getModel('customer/customer')
		->setMobileCode($this->getRequest()->getPost('mobile_code'))
        ...
        try {
           ...
           // if (empty($errors)) {
            ...
            if ($e->getCode() === Mage_Customer_Model_Customer::EXCEPTION_EMAIL_EXISTS) {
				...
				$message = $this->__('This customer email already exists');
    ...
    {
		$login_type = Mage::app()->getRequest()->getParam('normal_login');
		...
		if($login_type == 'newcustomer')
		{
		$urlll = Mage::getBaseUrl();
		$this->_redirectUrl($urlll);
		}
		else
		{
    ...
    public function editPostAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
			...
			$customerData['username'] = $this->getRequest()->getPost('mobile');
            //print_r($customerData);exit;
			$customerData1 = Mage::getSingleton('customer/session')->getCustomer();
            $firstnamechk = $customerData1->getFirstname();
			$lastnamechk = $customerData1->getLastname();
            $emailchk = $customerData1->getEmail();
            $mobilechk = $customerData1->getMobile();
			if($firstnamechk != $this->getRequest()->getParam('firstname'))
			{
				$emaildata['first']=  "First Name:".$this->getRequest()->getParam('firstname');
			}
			if($lastnamechk != $this->getRequest()->getParam('lastname'))
			{
				$emaildata['lastname']=  "Last Name:".$this->getRequest()->getParam('lastname');
			}
			if($emailchk != $this->getRequest()->getParam('email'))
			{
				$emaildata['email']=  "Email:".$this->getRequest()->getParam('email');
			}
			if($mobilechk != $this->getRequest()->getParam('mobile'))
			{
				$emaildata['mobile']=  "Mobile No:".$this->getRequest()->getParam('mobile');
			}
			
			$countmail= count($emaildata);
			if($countmail >= 1){
					$mailTemplate1 = Mage::getModel('core/email_template');
			        $templateId1 =3; //template id for sending customer data
					$template_collection1 =  $mailTemplate1->load($templateId1);
					$template_data = $template_collection1->getData();
					$templateId1 = $template_data1['template_id'];
					$mailSubject1 = $template_data1['template_subject'];  
					$from_email1 = Mage::getStoreConfig('trans_email/ident_sales/email'); //fetch sender email
					$from_name1 = Mage::getStoreConfig('trans_email/ident_sales/name'); //fetch sender name
					$sender1 = array('name'  => 'msupply',
									'email' => 'no-reply@msupply.com');                                
					$vars1 = array('first'=>$emaildata['first'],'lastname'=>$emaildata['lastname'],'eamil'=>$emaildata['email'] ,'mobile'=>$emaildata['mobile'] ,'name'=>$firstnamechk); 
					$email1 = $this->getRequest()->getPost('email');         
					$storeId1 = Mage::app()->getStore()->getId();
					$model1 = $mailTemplate1->setReplyTo($sender['email'])->setTemplateSubject($mailSubject);
					$model1->sendTransactional($templateId1, $sender1,$email1, 'msupply', $vars1, $storeId1);
			}
            ...
            if ($customerErrors !== true) {
                ...
                if ($this->getRequest()->getParam('change_password')) {
                    ...
                    $name = $customerData->getId();
                    $currPass   = $this->getRequest()->getPost('current_password');
                    $newPass    = $this->getRequest()->getPost('password');
                    $confPass   = $this->getRequest()->getPost('confirmation');
					$mailemail   = $this->getRequest()->getPost('email');
					$mailfirstname   = $this->getRequest()->getPost('firstname');
					...
					$sender = array('name'  => 'msupply',
									'email' => 'no-reply@msupply.com');                                
					$vars = array('newpassword'=>$newPass,'firstname'=>$mailfirstname ); 
					$email = $this->getRequest()->getPost('email');         

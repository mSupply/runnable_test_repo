...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
        ...
        $payment_code = $this->getPayment()->getMethodInstance()->getCode();
       if(($payment_code == 'cheque_checkout') && ($order_status == 'holded')){
		   $label_msg = 'Your Order is currently "<b>On Hold</b>". It will be confirmed once Cheque/DD is cleared.';
		   $label_sub= 'Placed';
		}else{
	      $label_msg = 'We will send you another email once the items are dispatched.';
	      $label_sub= 'Confirmation';
		}
        ...
        //Mage::log('mail status '.$this->getStatus().'mail pay code'.$this->getPayment()->getMethodInstance()->getCode());
        ...
        $mailer->setTemplateParams(array(
            ...
            'custom_label' => $label_msg,
            'custom_subject' => $label_sub
        ));
        ...
        if(($payment_code == 'cheque_checkout') && ($order_status == 'holded')){
         /** @var $mailer Mage_Core_Model_Email_Template_Mailer */
        $mailer_fullfill = Mage::getModel('core/email_template_mailer');
	   ...
	   foreach($var as $row)
		{
		       // Mage::log('mail '.$row);
                ...
                $emailInfo1->addTo($row);
                $mailer_fullfill->addEmailInfo($emailInfo1);
            }
      
       
        Mage::log('mail status1 '.$this->getStatus().'mail pay code1'.$this->getPayment()->getMethodInstance()->getCode());

        // Set all required params and send emails
        $mailer_fullfill->setSender(Mage::getStoreConfig(self::XML_PATH_EMAIL_IDENTITY, $storeId));
        $mailer_fullfill->setStoreId($storeId);
        $mailer_fullfill->setTemplateId(10);
        $mailer_fullfill->setTemplateParams(array(
            'order'        => $this,
            'billing'      => $this->getBillingAddress(),
            'payment_html' => $paymentBlockHtml,
            'custom_label' => $label_msg,
        ...
        $mailer_fullfill->setQueue($emailQueue)->send();
        //$mailer_fullfill->send();
        }

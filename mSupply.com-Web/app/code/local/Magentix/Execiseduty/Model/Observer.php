...
class Magentix_Execiseduty_Model_Observer
{

    /**
     * Set execiseduty amount invoiced to the order
     *
     * @param Varien_Event_Observer $observer
     * @return Magentix_Execiseduty_Model_Observer
     */
    public function invoiceSaveAfter(Varien_Event_Observer $observer)
    {
        $invoice = $observer->getEvent()->getInvoice();

        if ($invoice->getBaseExecisedutyAmount()) {
            $order = $invoice->getOrder();
            $order->setExecisedutyAmountInvoiced($order->getExecisedutyAmountInvoiced() + $invoice->getExecisedutyAmount());
            $order->setBaseExecisedutyAmountInvoiced($order->getBaseExecisedutyAmountInvoiced() + $invoice->getBaseExecisedutyAmount());
        }
    ...
    /**
     * Set execiseduty amount refunded to the order
     *
     * @param Varien_Event_Observer $observer
     * @return Magentix_Execiseduty_Model_Observer
     */
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($creditmemo->getExecisedutyAmount()) {
            $order = $creditmemo->getOrder();
            $order->setExecisedutyAmountRefunded($order->getExecisedutyAmountRefunded() + $creditmemo->getExecisedutyAmount());
            $order->setBaseExecisedutyAmountRefunded($order->getBaseExecisedutyAmountRefunded() + $creditmemo->getBaseExecisedutyAmount());
        }
    ...
    public function updatePaypalTotal(Varien_Event_Observer $observer)
    {
        ...
        $cart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_SUBTOTAL, $cart->getSalesEntity()->getExecisedutyAmount());
	...
	$total_amount_setting = Mage::getStoreConfig('execiseduty/execiseduty_group/freeshippingtotal');
	$total_minimum_setting = Mage::getStoreConfig('execiseduty/execiseduty_group/minimumshippingamount');
	$total_each_setting = Mage::getStoreConfig('execiseduty/execiseduty_group/shipamounteach');

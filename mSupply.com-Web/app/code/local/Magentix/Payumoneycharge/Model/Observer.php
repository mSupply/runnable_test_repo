...
class Magentix_Payumoneycharge_Model_Observer
{

    /**
     * Set payumoneycharge amount invoiced to the order
     *
     * @param Varien_Event_Observer $observer
     * @return Magentix_Payumoneycharge_Model_Observer
     */
    public function invoiceSaveAfter(Varien_Event_Observer $observer)
    {
        $invoice = $observer->getEvent()->getInvoice();

        if ($invoice->getBasePayumoneychargeAmount()) {
            $order = $invoice->getOrder();
            $order->setPayumoneychargeAmountInvoiced($order->getPayumoneychargeAmountInvoiced() + $invoice->getPayumoneychargeAmount());
            $order->setBasePayumoneychargeAmountInvoiced($order->getBasePayumoneychargeAmountInvoiced() + $invoice->getBasePayumoneychargeAmount());
        }
    ...
    /**
     * Set payumoneycharge amount refunded to the order
     *
     * @param Varien_Event_Observer $observer
     * @return Magentix_Payumoneycharge_Model_Observer
     */
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($creditmemo->getPayumoneychargeAmount()) {
            $order = $creditmemo->getOrder();
            $order->setPayumoneychargeAmountRefunded($order->getPayumoneychargeAmountRefunded() + $creditmemo->getPayumoneychargeAmount());
            $order->setBasePayumoneychargeAmountRefunded($order->getBasePayumoneychargeAmountRefunded() + $creditmemo->getBasePayumoneychargeAmount());
        }
    ...
    public function updatePaypalTotal(Varien_Event_Observer $observer)
    {
        ...
        $cart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_SUBTOTAL, $cart->getSalesEntity()->getPayumoneychargeAmount());

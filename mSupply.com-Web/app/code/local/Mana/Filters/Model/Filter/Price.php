    ...
    implements Mana_Filters_Interface_Filter
{
    ...
    public function getMinPriceInt() {
        if (!$this->_isMinPriceIntCalculated) {
            ...
            $query = $this->getQuery();
            $queryResult = $query->getFilterRange($this->getFilterOptions()->getCode());
            $this->_minPriceInt = $queryResult['min'];

            if (!$this->_minPriceInt && $this->_getIsFilterable() == 2) {
                $rootCategory = Mage::getModel('catalog/category')
                    ->setStoreId(Mage::app()->getStore()->getId())
                    ->load(Mage::app()->getStore()->getRootCategoryId());
                $currentCategory = $this->getLayer()->getCurrentCategory();
    ...
    {
        $min = $this->_getResource()->getMinPriceOnCollection($this, $collection);
        $min=$this->_floor($min);
        $this->setData('min_price_int', $min);
    ...
    protected function _floor($value) {
        return floor($value);
    }
    /**
